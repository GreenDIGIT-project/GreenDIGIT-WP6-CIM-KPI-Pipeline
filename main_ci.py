...
def wattnet_fetch(lat: float, lon: float, start: datetime, end: datetime, aggregate: bool = True, extra_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if 300 <= r.status_code < 400:
        ...
        print("[wattnet_fetch] redirect:", r.status_code, "->", location, flush=True)
        raise HTTPException(status_code=502, detail=f"WattNet redirect {r.status_code} to {location or 'unknown'}")
    if not r.ok:
    ...
    try:
        ...
        snippet = r.text[:300]
        print("[wattnet_fetch] non-json response:", snippet, flush=True)
        raise HTTPException(status_code=502, detail=f"WattNet non-JSON response (status {r.status_code})")
