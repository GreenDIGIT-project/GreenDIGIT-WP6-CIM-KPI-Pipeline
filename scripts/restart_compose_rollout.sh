...
compose_has_service() {
  local service="$1"
  "${COMPOSE_BIN[@]}" config --services | rg -x "$service" >/dev/null
}
...
wait_for_http_status() {
  ...
  while (( SECONDS < deadline )); do
    local code
    if [[ -n "$body" ]]; then
      code="$(curl -sS -o /dev/null -w '%{http_code}' -m 4 -X "$method" -H "Content-Type: ${content_type}" --data "$body" "$url" || true)"
    else
      code="$(curl -sS -o /dev/null -w '%{http_code}' -m 4 -X "$method" "$url" || true)"
    fi
...
wait_kpi_endpoints() {
  ...
  wait_for_http_status "kpi-cfp" "${base}/cfp?ci=100&pue=1.2&energy_wh=1000" "200,401,422" "$KPI_TIMEOUT_SECONDS"
}
...
rollout_remaining_services() {
  ...
  for svc in "${services[@]}"; do
    ...
    case "$svc" in
      cim-service)
        wait_for_http_status "cim-service" "http://127.0.0.1:8012/" "200,307,404" 180 || true
        ;;
      sql-adapter)
        wait_for_http_status "sql-adapter" "http://127.0.0.1:8033/docs" "200,307" 180 || true
        ;;
      grafana)
        wait_for_http_status "grafana" "http://127.0.0.1:8044/login" "200" 180 || true
        ;;
      metrics-db-dirac)
