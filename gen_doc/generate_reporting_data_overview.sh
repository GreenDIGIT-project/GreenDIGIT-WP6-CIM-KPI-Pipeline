...
# plus a raw-SQL integrity summary from monitoring.fact_site_event/detail_*.
# Defaults: activity=grid, auto-select best site by longest coverage then continuity.
#
# Usage:
#   ./gen_doc/generate_reporting_data_overview.sh
#   ./gen_doc/generate_reporting_data_overview.sh --site SITE_NAME
...
ACTIVITY="grid"
OUTPUT="${SCRIPT_DIR}/reporting_data_overview.json"
SUMMARY_OUTPUT="${SCRIPT_DIR}/reporting_data_overview.csv"
TEX_OUTPUT="${SCRIPT_DIR}/reporting_data_overview.tex"
SITE_FILTERS=()
...
if [[ ${#SITE_FILTERS[@]} -eq 0 && "${ACTIVITY}" == "grid" ]]; then
...
if [[ "${ACTIVITY}" == "all" ]]; then
  ACTIVITY="cloud,grid,network"
fi
...
if [[ ${#SITE_FILTERS[@]} -gt 0 ]]; then
  ...
  SITE_ARRAY_SQL="NULL::text[]"
fi

IFS=',' read -r -a ACTIVITY_FILTERS <<< "${ACTIVITY}"
ACTIVITY_ARRAY_SQL="ARRAY["
for activity in "${ACTIVITY_FILTERS[@]}"; do
  trimmed_activity="$(echo "${activity}" | xargs)"
  ACTIVITY_ARRAY_SQL="${ACTIVITY_ARRAY_SQL}'${trimmed_activity//\'/''}',"
done
ACTIVITY_ARRAY_SQL="${ACTIVITY_ARRAY_SQL%,}]::text[]"
...
SELECT
  ...
  || ' RI site time series across the selected monitoring activity classes.'
  || E'\n\n'
  ...
  SELECT jsonb_build_object(
    ...
    'metadata_after_anonymisation', jsonb_build_object(
      ...
      'configuration_tags', jsonb_build_array(
        ...
        'aggregation:15m',
        CASE WHEN '${ANONYMIZE}' = 'true' THEN 'anonymised:true' ELSE 'anonymised:false' END
      ),
      'site_anonymised_id', (
        SELECT CASE
